...
app._initializeHTTP = function() {
  ...
  // Get all peers
  this.get('/listAllPeers', function(req, res, next) {
    //get all peer connected to the server 
    var peers = self._clients;

    if (!peers) 
      return res.json(JSON.stringify("No hay datos"));

    // var peersKeys = Object.keys(peers); 
    // var requestPeers = [];

    // for (var i = peers.length - 1; i >= 0; i--) {
    //   requestPeers.push(peers[i]);
    // }

    // res.json(peers);
    // json = JSON.parse(peers);
