...
app._initializeWSS = function(server) {
  ...
  // TODO: al hacer la conexión con el websocket (no entre peers), ya pasar el nombre del usuario y guardarlo en un mapa.
  this._wss.on('connection', function(socket) {
    ...
    if (!self._clients[key] || !self._clients[key][id]) {
      self._checkKey(key, ip, function(err) {
        if (!err) {
          // Agrego un usuario nuevo al listado de clients
          if (!self._clients[key][id]) {
            self._clients[key][id] = { token: token, ip: ip }; // TODO: Agregar en el objeto del cliente el nombre de usuario
            self._ips[ip]++;
...
app._initializeHTTP = function() {
  ...
  this.get('/countPeers', jsonParser, function(req, res, next) {
    var wss = self._wss;
    //get all peer connected to the server 
    var peers = wss.clients;

    if (!peers) 
      return res.json(JSON.stringify("0"));

    console.log(peers);
    // TODO: El error que ocurre es que no puede convertir el objeto clients (List[WebSocket]) a cadena.
    // res.json(JSON.stringify(peers));

    // Devuelve la cantidad de clientes conectados
    res.json(JSON.stringify(peers.length));
  });

  // GetAllPeers GetListAllPeers Listado de clientes conectados
  this.get('/listAllPeers', jsonParser, function(req, res, next) {
    var wss = self._wss;
    //get all peer connected to the server 
    var peers = wss.clients;
